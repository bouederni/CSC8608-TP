...
prompt = st.text_area("Prompt", value="ultra-realistic e-commerce product photo of a black leather minimalist backpack with silver zippers, front pocket and top handle, isolated on a pure white studio background, professional softbox lighting, soft shadow underneath, centered composition, highly detailed texture, sharp focus, commercial catalog style, 8k resolution")
...
if mode == "Img2Img":
    ...
    if up is not None:
        ...
        st.image(init_image, caption="Input image", width='content')
...
if run:
    ...
    st.image(img, caption=f"{config['mode']} | {config['scheduler']} | seed={config['seed']}", width='content')
