...
CKPT_PATH = "models/sam_vit_h_4b8939.pth"   # à adapter
MODEL_TYPE = "vit_h"
...
# 2) Affichage image
st.image(img, caption=f"{img_name} ({W}x{H})", width='content')
...
def draw_preview(image_rgb: np.ndarray, box_xyxy: np.ndarray, points):
    ...
    # Curseur
    size = 12
    cv2.line(bgr, (int(px) - size, int(py)), (int(px) + size, int(py)), (0, 0, 0), 2)
    cv2.line(bgr, (int(px), int(py) - size), (int(px), int(py) + size), (0, 0, 0), 2)

    return cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)

preview = draw_preview(img, box, st.session_state["points"])
st.image(preview, caption="Prévisualisation : bbox + points (avant segmentation)", width='content')
...
if lp is not None and lp["img_name"] == img_name:
    ...
    st.image(overlay, caption=f"mask_idx={idx} | score={float(scores[idx]):.3f}", width='content')
    st.write({
        "mask_idx": int(idx),
